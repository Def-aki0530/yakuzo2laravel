			...
			<table class="text-xl mx-fixed mx-auto">
				...
				<tr class="h-10">
							...
							<select name="tenpo_code">
								@foreach( $tenpo_list as $tenpo)
									<option value="{{ $tenpo->torihikisaki_code}}"
										@if($tenpo->torihikisaki_code == old('tenpo_code',$tenpo_code)) selected @endif>
					...
					<td class="text-left">
						<select name="hatchu_kbn">
							...
							<option value="3" @if( old('hatchu_kbn',$hatchu_kbn) == '3') selected @endif>ＥＰＩ</option>
						</select>
					</td>
				</tr>		
				<tr class="h-10">
				...
				<tr class="text-left">
					...
					<td class="text-center" colspan="3">
						...
						<button 
							type="button" onclick="registCancel()">キャンセル</button>
					</td>
				</tr>
			</table>
		</form>
