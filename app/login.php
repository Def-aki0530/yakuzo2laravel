...
class Login
{
    ...
    public function check(){
        ...
        $result = DB::table("mst_shain as ms")->join("mst_kengen as mk", "ms.kengen_code", "mk.kengen_code")
                  ...
                  ->where("ms.password", $this->data["password"])
                  ->get();

        if($result->count() == 0){
            $this->data["msg"] = "社員コード、またはパスワードに間違いがあります。";
            return false;
        }
        ...
        if($this->data["kengen_code"] == "002" &&  $this->data["tenpo_code"] == ""){
            $this->data["msg"] = "店舗権限者は、店舗の選択が必須です。";
            return false;
        }
        ...
        if($this->data["kengen_code"]  == "001"){
            ...
            $result = Torihikisaki::select("torihikisaki_name")
                      ->where("torihikisaki_code", $this->data["tenpo_code"])->first();
            
            $this->data['tenpo_name'] = $result->torihikisaki_name;
        }
